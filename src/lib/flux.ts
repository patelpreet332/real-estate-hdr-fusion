...
export async function generateFluxImage(prompt: string) {
  if (!BFL_API_KEY || BFL_API_KEY === "your_api_key_here") {
    return { success: false, error: "BFL API Key is missing. Please check your .env.local file." };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let errorMessage = `Flux Submission Failed: ${msg}`;
      if (response.status === 401) errorMessage = "Invalid BFL API Key. Please verify your credentials.";
      if (response.status === 429) errorMessage = "BFL API rate limit exceeded. Please wait a moment.";
      if (response.status === 402) errorMessage = "BFL account balance low. Please check your BFL dashboard.";
    ...
    if (!taskData.id) {
      return { success: false, error: "BFL API did not return a Task ID. Please try again." };
    }
    ...
    console.error("Flux Generation Error Details:", error);
    return { success: false, error: errorMessage };
  }
}
...
async function pollFluxResult(taskId: string): Promise<string> {
  ...
  throw new Error("BFL generation timed out. The model might be overloaded. Please try again.");
}
